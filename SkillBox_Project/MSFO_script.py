...
def read_content(filename):
    ...
    content_pages_raw = camel(filename, pages='3-20',
                              table_areas=['50,800,550,40'])  # now dict with page info
...
def camel(filename, pages, table_areas, st=""):
    pages_list_raw = list()
    pdf_file = camelot.read_pdf(filename, flavor="stream",
                                pages=pages, table_areas=table_areas,
                                strip_text=st, edge_tol=200)
...
def identify_info(ticker, pages_list_raw):
    ...
    for page in pages_list_raw:
        ...
        for number, line_info in page:  # line looks like (number, {number1:column1, number2:column2 ...})
            ...
            if len([i for i in line_info.values() if i != ""]) == 1 and line_info_list[0][0].islower() is False:
    ...
    for page in ticker.pages:
        ...
        if ticker.pribyl == 0:
            for name in pribyl_list:
                if page.get(name) is not None and ticker.pribyl == 0:
...
def write_db_postsql(ticker):
    ...
    cur = conn.cursor()
    # Execute a query
    cur.execute("INSERT into ticker_info \
                (url, name, pribyl, viruchka, price, amount, measuring_unit, pe, ps) \
